...
# DATA PATH
# -------------------------
DATA_PATH = r"E:\project labmentix\ola ride project\ola_cleaned.csv"  # local file path
...
try:
    ...
    con.execute("CREATE TABLE rides AS SELECT * FROM df")
    # Example check
    result = con.execute("SELECT COUNT(*) AS total_rides FROM rides").fetchdf()
    st.write("Total rows in DuckDB table:", result["total_rides"][0])

except FileNotFoundError:
    st.error(f"⚠️ File not found at: {DATA_PATH}")
