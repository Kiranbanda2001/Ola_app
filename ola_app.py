...
st.markdown(
    "<h1 style='text-align:center;color:#000;background:linear-gradient(90deg,#FDD835,#4A4A4A);padding:20px;border-radius:15px;'>🚖 Ola Rides — Interactive Analytics Dashboard</h1>",
    unsafe_allow_html=True
)

uploaded_file = st.file_uploader("Upload your Ola dataset (CSV)", type=["csv"])
...
if df is not None:
    ...
    with col1:
        st.metric("Total Rides", len(df_filtered))
    with col2:
        cancelled = df_filtered["Booking_Status_norm"].str.contains("Cancel", case=False, na=False).sum()
        cancel_pct = f"{(cancelled/len(df_filtered)*100):.2f}%" if len(df_filtered) > 0 else "N/A"
        st.metric("Cancellation %", cancel_pct)
    with col3:
        revenue = df_filtered[df_filtered["Booking_Status_norm"]=="Success"]["Booking_Value"].sum()
        st.metric("Revenue (Success)", f"₹{revenue:,.0f}")
